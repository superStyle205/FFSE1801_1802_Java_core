...
public class QuanLySinhVien {
	...
	public void themSinhVien() throws Exception{
		...
		System.out.println("Lỗi ~~~~ id rong ......." + e);
	}catch (Exception exception) {
		System.out.println("loiox~~~ " + exception);
	}
		}
	}
	private void kiemTra(SinhVien sinhVien) throws Exception{
		//kiểm tra tên
		if (sinhVien.getName().equals("")) {
			throw new Exception("Không được để trống :");
		}
		//kiểm tra tuổi 
		if (sinhVien.getAge() < 0 || sinhVien.getAge() > 100) {
			throw  new Exception("không được nhỏ hơn 0 và lớn hơn 100");
		}
		//kiểm tra DDiemere Trung bình 
		if (sinhVien.getGpa() < 0.0 || sinhVien.getGpa() > 10.0) {
			throw new Exception("không được nhỏ hơn 0.0 và lớn hơn 10.0");
		}
	}
	private void kiemTraId(int id)throws Exception{
		int xet = 1;
		for (int i = 0; i < sinhVienArray.length; i++) {
			if (sinhVienArray[i].getId() == id) {
				xet = 1;
			} else if (xet == 1) {
				throw new Exception("không để trống id!!");
			}
		}
	}
